...
from data.QuantDataApi import QuantDataApi as WXDBReader
...
class FactorAnalyserBase(object):
    ...
    def prepare_data(self):
        ...
        for date in self.recalculate_date_list:
            ...
            if self.customized_universe is None:
                ...
                # 按日期记录因子数据到dict里
                self.raw_universe_df_dict[date] = WXDBReader.get_universe(self.universe, date, self.factor_list)
    ...
    def grouping_test(self, group_num, control_dict, group_by_benchmark=False, weight_method='LVW', max_stock_num=30):
        ...
        industry_alpha_list = []
        for i, recalculate_date in enumerate(self.recalculate_date_list):
